...
void MD5::append(const void *inBuf, size_t inLen) {
    MD5Update(&m_ctx, (md5byte*)inBuf, inLen);
}

void MD5::append(const std::string& str) {
    append((unsigned char*) str.c_str(), (unsigned int) str.length());
}

void MD5::getDigest(unsigned char digest[16]) {
    MD5Final(digest, &m_ctx);
}

void MD5::getDigest(std::string& digest) {
    unsigned char szBuf[16] = {'\0'};
    getDigest(szBuf);
    char szResult[33] = {0};
    sprintf(szResult, "%02X%02X%02X%02X%02X%02X%02X%02X"\
                  ...
                  szBuf[15]);
    digest.append(szResult);
}

std::string MD5::getMD5(const std::string& text) {
    if (0 == text.size()) {
        return "";
    }
...
std::string MD5::getFileMD5(const std::string& file) {
    if (0 == file.size()) {
        return "";
    }
    ...
    while ((nBytes = fread(buffer, 1, 1024, pFile)) != 0) {
